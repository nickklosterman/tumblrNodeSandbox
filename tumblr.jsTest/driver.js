...
//if need another input, check for it and store it
if (choice === 17) {
    if (process.argv[3]) {
	var tumblrListFilename = process.argv[3];
	console.log(tumblrListFilename);
    }
}
switch(choice){
case 1:
...
case 16:
    ...
    break;
case 17: //read tumblr names from a file and process
    ...
    var that = this;
    if (typeof tumblrListFilename !== 'undefined') {
	console.log(tumblrListFilename);
	fs.exists(tumblrListFilename,function(exists) {
	    if (!exists){
		console.log(tumblrListFilename+" doesn't exit") ;
	    } else  {
		fs.readFile(tumblrListFilename,'utf8', function (err, data) {
		    if (err) throw err;
		    console.log(data)
		    var temp = data.split('\n')
